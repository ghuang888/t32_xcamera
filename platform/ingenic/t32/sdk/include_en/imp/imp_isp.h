...
typedef enum {
	...
	IMPISP_INPUT_ALLCACHED_MODE,	/**< multi sensor mode with sensor cached */
...
typedef struct	{
	...
	IMPISPMultiInputMode input;				/**< the cache mode when enable dual sensor mode */
	IMPISPMultiSplitJoint joint;			/**<  Multi-camera splicing mode (reserved)*/
 ...
 */
typedef enum {
	IMPISP_ANTIFLICKER_FREQ_50HZ,			/**< Antiflicker for 50Hz */
	IMPISP_ANTIFLICKER_FREQ_60HZ,			/**< Antiflicker for 60Hz */
...
typedef struct {
	...
	uint64_t list[15];				/**< Target brightness Indicates the target brightness of the table */
} IMPISPAeAtList;
 ...
 * @attention Before using that function, make sure that ISP is working properly.
 */
int32_t IMP_ISP_SetOSDBlock(IMPVI_NUM num, IMPISPOSDBlockAttr *attr);
 ...
 * @attention Before using that function, make sure that ISP is working properly.
 */
int32_t IMP_ISP_GetOSDBlock(IMPVI_NUM num, IMPISPOSDBlockAttr *attr);
...
/**
 * @fn int32_t IMP_ISP_GetDrawBlock(IMPVI_NUM num, IMPISPDrawBlockAttr *attr)
 *
 * Get draw Attr.
 *
 * @param[in]	num			The sensor num label.
 ...
 * @attention Before using that function, make sure that ISP is working properly.
 */
int32_t IMP_ISP_GetDrawBlock(IMPVI_NUM num, IMPISPDrawBlockAttr *attr);

/**
 * @fn int32_t IMP_ISP_SetDrawBlock(IMPVI_NUM num, IMPISPDrawBlockAttr *attr)
 *
 * Set draw Attr.
 *
 * @param[in]	num			The sensor num label.
 ...
 * @attention Before using that function, make sure that ISP is working properly.
 */
int32_t IMP_ISP_SetDrawBlock(IMPVI_NUM num, IMPISPDrawBlockAttr *attr);
