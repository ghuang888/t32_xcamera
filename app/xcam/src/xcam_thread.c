...
int xcam_thread_create(char * name, void *(*start_routine) (void *), void *param)
{
	...
	// param.sched_priority = 99; // 设置为较低的优先级
    param_x.sched_priority = sched_get_priority_max(SCHED_RR); // 设置为最高的实时优先级

    if (pthread_setschedparam(threads_info[tid].handle_thread, SCHED_RR, &param_x) != 0) {
