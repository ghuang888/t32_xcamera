...
int main(int argc, char *argv[]) {
    ...
    // while(true){
    /* Step.6 Start to ivs */
    ret = sample_ivs_personvehicledet_start(0, 2, &interface);
    if (ret < 0) {
...
// #ifdef SETPARAM
        ...
        if(ii%1 == 0){//间隔20帧更新一下参数
            ...
            int retk;
            personvehicledet_param_input_t param;
            if(retk = interface->getParam(interface, (personvehicledet_param_input_t *)(&param)) < 0){//if (interface->getParam && ((retk = interface->getParam(interface, &param) < 0))){
                IMP_LOG_ERR(TAG, "interface->getParam failed,ret=%d\n", retk);
                return -1;
            }
            param.sense = 4;
            param.enable_move = false;
            if(retk = interface->setParam(interface, (personvehicledet_param_input_t *)(&param)) < 0){//if (interface->setParam && ((retk = interface->setParam(interface, &param) < 0))){
        ...
        printf("isperson: %d\n", r->isperson);
        for(int i = 0; i < r->count; i++) {
            if ((r->isperson && r->personvehicle[i].class_id == 0) || r->personvehicle[i].class_id != 0){
                rrr++;
                int track_id = r->personvehicle[i].track_id;
                int class_id = r->personvehicle[i].class_id;
                float confidence = r->personvehicle[i].confidence;
    ...
    /* Step.8 stop to ivs */
    ret = sample_ivs_personvehicledet_stop(2, interface);
    if (ret < 0) {
