...
::MultiFramedRTPSource(UsageEnvironment& env, Groupsock* RTPgs,
  ...
  increaseReceiveBufferTo(env, RTPgs->socketNum(), 1*1024*1024);
}
...
void MultiFramedRTPSource::doGetNextFrame1() {
 ...
 // printf("####### %s,%s,%d: nextPacket = %p, packetLossPrecededThis = %d\n",
//		  __FILE__, __func__, __LINE__, nextPacket, packetLossPrecededThis);
  ...
  //printf("####### %s,%s,%d:specialHeaderSize = %d \n",
	//	  __FILE__, __func__, __LINE__, specialHeaderSize);
    }

    // Check whether we're part of a multi-packet frame, and whether
    // there was packet loss that would render this packet unusable:
    if (fCurrentPacketBeginsFrame) {
	//	printf("####### %s,%s,%d:fPacketLossInFragmentedFrame = %d, packetLossPrecededThis = %d\n", 
	//			__FILE__, __func__, __LINE__, fPacketLossInFragmentedFrame, packetLossPrecededThis);
    ...
    if (fPacketLossInFragmentedFrame) {
      ...
      fReorderingBuffer->releaseUsedPacket(nextPacket);
      fNeedDelivery = True;
	  printf("\033[47;31mThis packet is unusable; reject it.\033[0m\n");
      continue;
    }
  ...
  //printf("####### %s,%s,%d: fCurrentPacketCompletesFrame = %d, fFrameSize = %d, fNumTruncatedBytes = %d\n",
//		  __FILE__, __func__, __LINE__, fCurrentPacketCompletesFrame, fFrameSize, fNumTruncatedBytes);
...
void MultiFramedRTPSource::networkReadHandler1() {
  ...
  //printf("####### %s,%s,%d: fPacketReadInProgress = %p\n", __FILE__, __func__, __LINE__, fPacketReadInProgress);
  if (bPacket == NULL) {
  ...
  do {
    ...
    Boolean packetReadWasIncomplete = fPacketReadInProgress != NULL;
	  //printf("####### %s,%s,%d: packetReadWasIncomplete = %d\n", __FILE__, __func__, __LINE__, packetReadWasIncomplete);
	...
	unsigned char* testbuf = bPacket->data();
	//printf("####### %s,%s,%d: packet: 0x%2x, 0x%2x\n",
	//		__FILE__, __func__, __LINE__, testbuf[0], testbuf[1]);
	//printf("####### %s,%s,%d: packetReadWasIncomplete = %d\n", __FILE__, __func__, __LINE__, packetReadWasIncomplete);
...
::getNextCompletedPacket(Boolean& packetLossPreceded) {
  ...
  if (fHeadPacket->rtpSeqNo() == fNextExpectedSeqNo) {
	...
	//printf("The very first packet is treated as if there was packet loss beforehand\n");
  ...
  if (fThresholdTime == 0) {
	...
	//printf("###### %s,%s,%d: uSecondsSinceReceived = %d, fThresholdTime = %d\n",
	//		__FILE__, __func__, __LINE__, uSecondsSinceReceived, fThresholdTime);
  }
  //printf("####### %s,%s,%d: fThresholdTime = [%p,%d], timeThresholdHasBeenExceeded = %d\n",
	//	  __FILE__, __func__, __LINE__, fThresholdTime, fThresholdTime, timeThresholdHasBeenExceeded);
