...
RTPInterface::RTPInterface(Medium* owner, Groupsock* gs)
  ...
  increaseSendBufferTo(envir(), fGS->socketNum(), 500*1024);
}
...
Boolean SocketDescriptor::tcpReadHandler1(int mask) {
  ...
  switch (fTCPReadingState) {
    ...
    case AWAITING_STREAM_CHANNEL_ID: {
      ...
      if (lookupRTPInterface(c) != NULL) { // sanity checksum
		  //fprintf(stderr, "Saw stream channel id: 0x%02x\n", c);
		  fStreamChannelId = c;
