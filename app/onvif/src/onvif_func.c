...
#define SMALL_INFO_LENGTH 512
	...
	sprintf(mac,"onvif://www.onvif.org/A1C2B3/mac/%02X%02X%02X%02X%02X%02X", HOST_MAC[0], HOST_MAC[1], HOST_MAC[2], HOST_MAC[3], HOST_MAC[4], HOST_MAC[5]);
	...
	strcat(ProbeMatches.ProbeMatch->Scopes->__item, mac);
...
{
	struct wsdd__ProbeMatchesType *probeMatches = wsdd__ProbeMatches;
    if (soap == NULL || probeMatches == NULL) {
        return soap_sender_fault(soap, "Invalid input parameter", NULL);
    }

    // 清除probeMatches结构体，准备填充数据
    soap_default_wsdd__ProbeMatchesType(soap, probeMatches);

    // 假设我们只返回一个匹配的设备
    struct wsdd__ProbeMatchType probeMatch;
    soap_default_wsdd__ProbeMatchType(soap, &probeMatch);

    // 填充probeMatch数据，例如设备类型、XAddrs等
    probeMatch.Types = (char*)soap_malloc(soap, sizeof("wsdd:Device")); // 设备类型
    strcpy(probeMatch.Types, "wsdd:Device");

    // 假设设备服务的地址是 "http://192.168.1.100:80"
    probeMatch.XAddrs = (char*)soap_malloc(soap, strlen("http://192.168.1.100:80") + 1);
    strcpy(probeMatch.XAddrs, "http://192.168.1.100:80");

    // 将probeMatch添加到probeMatches数组中
    probeMatches->__sizeProbeMatch = 1;
    probeMatches->ProbeMatch = (struct wsdd__ProbeMatchType*)soap_malloc(soap, sizeof(struct wsdd__ProbeMatchType) * probeMatches->__sizeProbeMatch);
    memcpy(probeMatches->ProbeMatch, &probeMatch, sizeof(probeMatch));
...
SOAP_FMAC5 int SOAP_FMAC6 __tds__GetDeviceInformation(struct soap* soap, struct _tds__GetDeviceInformation *tds__GetDeviceInformation, struct _tds__GetDeviceInformationResponse *tds__GetDeviceInformationResponse)
{
	...
	strcpy(tds__GetDeviceInformationResponse->Model, TDS_DEVICE_INFO_MODE);
	strcpy(tds__GetDeviceInformationResponse->FirmwareVersion, TDS_DEVICE_INFO_FW_VERSION);

	strcpy(tds__GetDeviceInformationResponse->SerialNumber, "00000000000");
...
SOAP_FMAC5 int SOAP_FMAC6 __trt__SetVideoEncoderConfiguration(struct soap* soap, struct _trt__SetVideoEncoderConfiguration *trt__SetVideoEncoderConfiguration, struct _trt__SetVideoEncoderConfigurationResponse *trt__SetVideoEncoderConfigurationResponse)
{
	...
	ret = func_video_set_resolution(frmChnNum, picWidth, picHeigth);
	if(ret < 0)
	{
	...
	frmRateDen = 1;
	ret = func_video_set_fps(frmRateNum,frmRateDen);
	if(ret < 0)
	{
	...
	ret = func_video_set_bitrate(encChnNum, targetBitRate);
	if(ret < 0)
	{
...
SOAP_FMAC5 int SOAP_FMAC6 __trt__GetStreamUri(struct soap* soap, struct _trt__GetStreamUri *trt__GetStreamUri, struct _trt__GetStreamUriResponse *trt__GetStreamUriResponse)
{
	...
	{
		sprintf(url, "rtsp://%s:%d/%s", ip, 8554, TT_MEDIA_0_NAME);
	}
	else if (0 == strcmp(trt__GetStreamUri->ProfileToken, "SubStreamToken"))
	{
		sprintf(url, "rtsp://%s:%d/%s", ip, 8554, TT_MEDIA_1_NAME);
	}
	else
	{
...
SOAP_FMAC5 int SOAP_FMAC6 __trt__DeleteOSD(struct soap* soap, struct _trt__DeleteOSD *trt__DeleteOSD, struct _trt__DeleteOSDResponse *trt__DeleteOSDResponse)
{
	...
	ret = func_osd_delete_osdrgn(streamnum, token);
	if (ret < 0) {
		return SOAP_ERR;
	}
