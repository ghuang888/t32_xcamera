...
int sample_system_init()
{
  ...
  memset(&(sensor_info[0].name), 0, 32);
  memcpy(&(sensor_info[0].name),g_sensor_name,strlen(g_sensor_name));
  memset(&(sensor_info[0].i2c.type), 0, 32);
  memcpy(&(sensor_info[0].i2c.type), g_sensor_name, strlen(g_sensor_name));

  sensor_info[0].i2c.addr=g_i2c_addr;
  sensor_info[0].default_boot=g_sboot;

  chn[0].fs_chn_attr.crop.width = g_width;
  chn[0].fs_chn_attr.crop.height = g_height;
  chn[0].fs_chn_attr.scaler.outwidth = g_width;
  chn[0].fs_chn_attr.scaler.outheight = g_height;
